    ...
    <div class="min-h-screen" :class="themeClasses.background">
        <main class="px-3 sm:px-4 md:px-6 lg:px-8 pb-6">
                    ...
                    <div class="flex items-center justify-between mb-2 gap-2">
                        ...
                        <div class="flex items-center space-x-1 sm:space-x-2 flex-shrink-0"
                            ...
                            <button @click.prevent="showSettings = !showSettings" :disabled="isChatBusy"
                                ...
                                :class="['inline-flex items-center justify-center p-2 rounded-full text-sm transition-shadow duration-200 transform focus:outline-none focus:ring-2', isChatBusy ? 'bg-gray-300 text-gray-600 cursor-not-allowed' : (isDarkMode ? 'bg-white text-gray-900 hover:shadow-md focus:ring-gray-300' : 'bg-blue-600 text-white hover:bg-blue-700 focus:ring-blue-300')]"
                                :aria-label="$t('aiSupport.settings')" role="button">
                                <svg xmlns="http://www.w3.org/2000/svg"
                                    class="h-5 w-5 text-current transform transition-transform duration-200 hover:scale-105"
                            ...
                            <div v-for="(message, index) in visibleMessages" :key="index" :class="[
                                ...
                                <div :class="[
                                    ...
                                    <div v-if="message && message.sources && message.sources.length > 0"
                                        class="mt-3 pt-3 border-t overflow-hidden"
                                            ...
                                            <li v-for="(source, si) in message.sources" :key="si"
                                                ...
                                                <a :href="source.url || source" target="_blank"
                                                    ...
                                                    <span v-if="source.title && source.url"
                                                        class="text-gray-500 ml-1 whitespace-nowrap">({{
            ...
            <div v-if="showSettings" class="fixed inset-0 z-50 flex items-center justify-center p-4">
                ...
                <div
                    :class="['relative w-full max-w-md mx-auto rounded-xl p-4 sm:p-5 shadow-xl ring-1 ring-black/6 transform transition-all duration-200 max-h-[90vh] overflow-y-auto', isDarkMode ? 'bg-gray-800 text-white' : 'bg-white text-gray-900']">

                    <div class="flex items-start justify-between mb-4">
                        <div class="flex items-start gap-3">
                            <div
                                :class="['p-2 rounded-md flex items-center justify-center', isDarkMode ? 'bg-gray-700 text-white' : 'bg-blue-50 text-blue-600']">
                                <!-- sliders icon -->
                        ...
                        <button @click.prevent="showSettings = false" aria-label="Close settings"
                            class="ml-3 p-2 rounded-full bg-red-600 text-white hover:bg-red-700 shadow-sm focus:outline-none focus:ring-2 focus:ring-red-300">
                    ...
                    <div v-if="isChatBusy"
                        class="mb-3 p-3 rounded text-sm bg-yellow-50 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-300 flex items-center space-x-3">
                    ...
                    <div class="space-y-3">
                        <div
                            :class="['p-3 rounded-lg border', isDarkMode ? 'bg-gray-900 border-gray-700' : 'bg-gray-50 border-gray-200']">
                            ...
                            <input type="range" min="0" max="2" step="0.01" v-model.number="stagedSettings.temperature"
                                :disabled="isChatBusy"
                                :class="['w-full h-2 rounded-lg bg-gray-200 accent-blue-500', isChatBusy ? 'opacity-60 cursor-not-allowed' : '']" />
                            <p class="text-xs mt-2" :class="isDarkMode ? 'text-gray-400' : 'text-gray-500'">{{
                                $t('aiSupport.temperatureDesc') }}</p>
                        </div>
                        ...
                        <div
                            ...
                            <input type="range" min="0" max="10" step="1" v-model.number="stagedSettings.maxSources"
                                :disabled="isChatBusy"
                                :class="['w-full h-2 rounded-lg bg-gray-200', isChatBusy ? 'opacity-60 cursor-not-allowed' : '']" />
                            <p class="text-xs mt-2" :class="isDarkMode ? 'text-gray-400' : 'text-gray-500'">{{
                                $t('aiSupport.maxSourcesDesc') }}</p>
                        </div>

                        <div
                            :class="['p-3 rounded-lg border', isDarkMode ? 'bg-gray-900 border-gray-700' : 'bg-gray-50 border-gray-200']">
                            <label class="block text-sm font-medium mb-2">{{ $t('aiSupport.maxTokens') }}</label>
                            <input type="number" min="64" step="1" v-model.number="stagedSettings.maxTokens"
                                :disabled="isChatBusy"
                                :class="['w-full px-3 py-2 rounded-md border', isChatBusy ? 'opacity-60 cursor-not-allowed' : '']" />
                            <p class="text-xs mt-2" :class="isDarkMode ? 'text-gray-400' : 'text-gray-500'">{{
                                $t('aiSupport.maxTokensDesc') }}</p>
                        </div>
                        ...
                        <div
                            ...
                            <input type="range" min="0" max="1" step="0.01" v-model.number="stagedSettings.topP"
                                :disabled="isChatBusy"
                                :class="['w-full h-2 rounded-lg bg-gray-200', isChatBusy ? 'opacity-60 cursor-not-allowed' : '']" />
                            <p class="text-xs mt-2" :class="isDarkMode ? 'text-gray-400' : 'text-gray-500'">{{
                                $t('aiSupport.topPDesc') }}</p>
                        </div>

                    </div>

                    <div class="mt-4 flex items-center justify-end space-x-2">
                        <button @click.prevent="resetAiSettings" :disabled="isChatBusy" :aria-disabled="isChatBusy"
                            :title="isChatBusy ? $t('aiSupport.settingsDisabled') : $t('aiSupport.reset')"
                            class="px-4 py-2 rounded-md border text-sm inline-flex items-center justify-center"
                            :class="[isDarkMode ? 'border-gray-600 text-gray-100 hover:bg-gray-700' : 'border-gray-200 text-gray-800 hover:bg-gray-100', isChatBusy ? 'opacity-60 cursor-not-allowed' : '']">{{
                            $t('aiSupport.reset') }}</button>
                        <button @click.prevent="applyAiSettings" :disabled="isChatBusy" :aria-disabled="isChatBusy"
                            :title="isChatBusy ? $t('aiSupport.settingsDisabled') : $t('aiSupport.done')"
                            class="px-4 py-2 rounded-md bg-blue-600 text-white hover:bg-blue-700 shadow-sm inline-flex items-center justify-center text-sm"
                            :class="isChatBusy ? 'opacity-60 cursor-not-allowed' : ''">{{ $t('aiSupport.done')
                            }}</button>
                    </div>
                </div>
            </div>
